...
def _verify_previewed_projection(
    plan: Mapping[str, object], files: Mapping[PurePosixPath, str]
) -> None:
    """Bind all generated bytes to the previously approved lifecycle preview."""
    previewed: dict[str, str] = {}
    for item in plan["managed_writes"]:
        if not isinstance(item, Mapping):
            raise UpgradeError("upgrade preview has an invalid managed write")
        if item.get("action") in {"create", "update"} and "sha256" in item:
            path, digest = item.get("path"), item.get("sha256")
            if not isinstance(path, str) or not isinstance(digest, str) or path in previewed:
                raise UpgradeError("upgrade preview has an invalid generated write")
            previewed[path] = digest
    actual = {
        relative.as_posix(): sha256(content.encode("utf-8")).hexdigest()
        for relative, content in files.items()
    }
    if previewed != actual:
        raise UpgradeError("generated projection changed after the upgrade preview; preview again")
